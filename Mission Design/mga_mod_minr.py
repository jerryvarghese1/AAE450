...
class mga_mod_minr:
    ...
    def __init__(self,
        ...
        if tof_encoding == 'alpha':
            ...
            if len(tof) != (len(seq) - 1):
                raise ValueError(
                    'When tof_encoding is direct, the tof must be a float (upper bound on the time of flight)' + str(
                        len(seq) - 1))
        ...
        self.dv_launch_weight = dv_launch_weight
        self.dv_arr_weight = dv_arr_weight
    ...
    def fitness(self, x):
        try:
            DVlaunch, DVfb, DVarrival, ls, _, _, _, _ = self._compute_dvs(x)

            xyz_list = []
            for i, l in enumerate(ls):
                xyz_list.append(self.lambert_xyz(l, N=500))

            tmp_rs = min(np.linalg.norm(np.vstack(xyz_list), axis=1))
            min_r = tmp_rs/25_362_000
            ...
            if self.multi_objective:
                ...
                return [self.dv_launch_weight*DVlaunch + np.sum(DVfb) + self.dv_arr_weight*DVarrival + np.abs((np.tanh(-20*(min_r - 2.75))+1)/(.00001*min_r))]
    ...
    def lambert_xyz(self, l, N=60, sol=0, units=1.0, color='b', legend=False, axes=None, alpha=1.):
        """
