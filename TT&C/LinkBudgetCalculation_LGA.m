function [LM_uplink,  LM_downlink] = LinkBudgetCalculation_LGA(sc_power, sc_freq, sc_Data_rate, DSN_diam, DSN_power, DSN_freq, DSN_Data_rate)
    %Inital Variables
    T0 = 290; %"Noise figure are always quoted at this reference temp"-SMAD
    eta_ka = 0.7; %overall efficiency for TWTA (SMAD page 637)
    R = 2;  %Range to Uranus in AU
...
%     Latm = -10; %atmospheric loss  (clear day) (unit: dB)
    ...
    etaLin_sca_sc = .7; % Antenna Efficiency spacecraft (Assume)
    Ltxrx_sc = -4; %Pointing loss spacecraft (assume)
    ...
    %User Termnial Type
%     beamwidth_down = 21./(f_down*D_sc); %half-power beamwidth (deg)
