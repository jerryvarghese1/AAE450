clear; clc;
%% Forward Link Case
    % anything with g is for ground station and sc is spacecraft
    % Values to be decided and/or subject to change:
    % (F , Coverage Area , Bit Rate , modem loss , Required Eb/N0, Rb
    % 
    % We can change any values to create plots like found in module 6 
%Inital Variables
...
Rb_SC = 200;%linspace(100, 10000, 100); %bits per seconds rate for S/C (500<->32000)-estimated from Juno
Rb_SC_dB = 10*log10(Rb_SC); %Data rate in dB-Hz
Rb_DSN = 1000;%linspace(100, 32000, 100); %bits per seconds rate for DSN (500<->32000)
Rb_DSN_db = 10*log10(Rb_DSN); %Data rate in dB-Hz

%% Know inputs Ground Station
dia_g = 34; %DSN Antennas either 34m or 70m
Ptx_g = 20000; %transmit power of ground (current estimate from SMAD) (W)
Lin_g = -2; %line loss for the ground station (Known), dB
Lbackoff_g = -2; %backoff loss (assumed from SMAD table 16-13), dB
...
dia_sc = 3; %Antennas on Spacecraft Bus (m)
Ptx_sc = 50; %51:150; %transmit power of SC (current estimate from SMAD) (W)
Lin_sc = -2; %Line loss spacecraft (estimated)
Lbackoff_sc = -2.5; %backoff loss (assumed from SMAD table 16-13)
Louttx_sc = Lin_sc + Lbackoff_sc; %backoff + line loss spacecraft (calc)
eta_sc = .7; % Antenna Efficiency spacecraft (Assume)
Ltxrx_sc = -5; %Pointing loss spacecraft (assume)
Ta_sc = 1000; %Antenna Noise Temp (assume) Kelvin
Tl_sc = 10; %Noise temperture (input from thermals)
T_sc = 70;  %Spacecraft avg. Temp (K)
F_sc = 1 + T_sc/T0 ; %SC Receiver Noise (assume)

%% Uplink(DSN to Spacecraft)

f_up = 8.5; %uplink for X band
...
Net_loss_up = Ls_up + Latm; %Atm loss + Space Loss
...
Ts_up = Ta_sc * L_up + (1-L_up) * Tl_sc + (F_sc-1) * T0;    %K
Ts_up_dB = 10*log10(Ts_up); %dbK

%Receiver G/T
G_T_sc = Gr_sc_up - Ts_up_dB; %(unit: dB/k)
Lcomb_up = Ls_up + Latm + Louttx_g + Ltxrx_sc;
...
%% Downlink, S/C to DSN
clc
f_down = 34.8; %downlink for KA band
...
Net_Loss_down = Latm + Ls_down; %Total Loss through space
...
Lcomb_down = Net_Loss_down + Louttx_sc + Ltxrx_g;
...
req_Eb_No = -3.0; %Required Eb/N0 SMAD table 16-12

ML_uplink = Eb_N0_up  + req_Eb_No %Link Margin Value
ML_downlink = Eb_N0_down + req_Eb_No
%% Plots
%plot for Transmit Power against Data Rate for a 4m antenna